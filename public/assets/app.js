...
function fetchPosts(url) {
    $.ajax({
        ...
        success: function (response) {
            ...
            if (lng == "ar") {
                $("#emp-info").text(
                    response.emp.Nom_ar_emp + " " + response.emp.Prenom_ar_emp
                );
            } else {
                $("#emp-info").text(
                    response.emp.Nom_emp + " " + response.emp.Prenom_emp
                );
            }
            populateTable(response.list_abs.data);
            displayPaginationInfo(
                response.list_abs.total,
                response.list_abs.last_page,
                response.list_abs.current_page
        ...
        if (post.statut == "NoJustier") {
            ...
            if (lng == "ar") {
                var row =
                    ...
                    post.heure_abs +
                    "</td>" +
                    "<td><a href=/Employe/read_just/" +
                    post.id_fichier +
                    ' target="_blank">التبرير </a></td>' +
                    "</tr>";
            } else {
                var row =
                    "<tr>" +
                            ...
                            success: function (responses) {
                                if (responses.code == 200) {
                                    uploadjust(
                                        ...
                                        response.data.sous_d
                                    );
                                    hideLoadingSpinner();
                                    alert(response.message);
                                    console.log(
                                        "add to stocke  ->" + responses.message
                                    );
                                } else {
                                    hideLoadingSpinner();
                                    alert(response.message);
...
function closeNav(absensform, id_nin, absens) {
    ...
    if (selectedB.length > 0) {
        ...
        console.log(
            "the data select are " + JSON.stringify(selectedValues[0].value)
        );
    ...
    $(document).ready(function () { 
        ...
        $("#aft").click(function (e) {
            ...
            if ($("#pv_inst").text() != "") {
                ...
                if (lng == "ar") {
                    $("#remq p").text(
                        "عليك التأكد من تاريخ التنصيب و تاريخ التوظيف"
                    );
                } else {
                    $("#remq p").text(
                        "Vous devez vérifier la date d'instalation et la date de recrutement"
                    );
                }
                $("#remq").addClass("remq");
...
$(document).ready(function () {
    ...
    var absens =
        '<i id="stdout-ic" class="fa fa-user-times" aria-hidden="true"></i>';
    var present =
        '<i id="stdin-ic" class="fa fa-check-circle" aria-hidden="true"></i>';
    ...
    $("#Dep").change(function () {
        ...
        console.log(
            "date" +
                $.isEmptyObject(dateabs) +
                "data " +
                JSON.stringify(dateabs)
        );
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        list_abs.employe.forEach(function (item) {
                            console.log(
                                "--" + JSON.stringify(list_abs.employe)
                            );
                            //  if()
                            dateabs.forEach(function (itemsdate) {
                                if (item.id_nin === itemsdate.id_nin) {
                                    item.absens = true;
                                    console.log(
                                        " i use for changing" + item.absens
                                    );
                                }
                            });
                        });
                        ...
                        list_abs.employe.forEach(function (item) {
                            if (item.absens) {
                                if (lng == "ar") {
                                    $("#AbsTable tbody").append(
                                        "<tr id=" +
                                            item.id_p +
                                            "><td><a href=/BioTemplate/search/" +
                                            item.id_nin +
                                            ">" +
                                            item.Nom_ar_emp +
                                            "</td><td>" +
                                            ...
                                            item.Nom_post_ar +
                                            "</td><td>" +
                                            item.Nom_sous_depart_ar +
                                            "</td><td>" +
                                            item.Nom_depart_ar +
                                            '</td><td id="stdin' +
                                            item.id_nin +
                                            '" class="std-handle abs">' +
                                            absens +
                                            "</td>" +
                                            '<td onclick="showDiv()" class="abs-info" id="abs' +
                                            item.id_nin +
                                            '"><i class="fa fa-exclamation-circle" aria-hidden="true"></i></td></tr>'
                                    );
                                } else {
                                    $("#AbsTable tbody").append(
                                        "<tr id=" +
                        ...
                        $("#AbsTable tbody tr").each(function () {
                            ...
                            $("#" + id_abs).click(function () {
                                ...
                                $("#AbsempTable tbody").empty();
                                if (lng == "ar") {
                                    $("#AbsempTable thead").append(
                                        '<tr><th style="text-align:justify">رقم</th>' +
                                    ...
                                    $("#AbsempTable thead").append(
                                        '<tr><th style="text-align:justify">Numero</th>' +
                                            ...
                                            $('input[name="CatjustRadio"]').on(
                                                ...
                                                function () {
                                                    if (
                                                        ...
                                                        console.log(
                                                            $(
                                                                'input[name="CatjustRadio"]:checked'
                                                            ).val()
                                                        );
                                                        dir = $(
                                                            'input[name="CatjustRadio"]:checked'
                                    ...
                                    $("#close").click(function () {
                                        ...
                                        if (jst && fil !== "") {
                                            ...
                                            if (nojst) {
                                                ...
                                                {
                                                    alert(
                                                        "Choisir le motif de justification"
                                                    );
                                                }
                                                $("#file").addClass(
                                                    "error-handle"
                    ...
                    success: function (response) {
                        ...
                        $("#AbsTable tbody tr").each(function () {
                            ...
                            $("#" + id_abs).click(function () {
                                ...
                                if (lng == "ar") {
                                    $("#AbsempTable thead").append(
                                        '<tr style="text-align: center;"><th>رقم</th>' +
                                            "<th>تاريخ الغياب</th>" +
                                            "<th>توقيت الغياب</th>" +
                                            "<th>سبب الغياب</th>" +
                                            "</tr>"
                                    ...
                                    $("#AbsempTable thead").append(
                                        '<tr style="text-align: center;"><th>Numero</th>' +
                                            "<th>Date d'bsence</th>" +
...
idinput.on("input", function () {
    // ✅ Ne garder que les chiffres (empêche d’écrire autre chose)
...
idinput.blur(function () {
    ...
    if (val.length === 18) {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.status == 200) {
                    ...
                    // ✅ Assainir les valeurs vides : si null, on met ''
                    $("#PHONE_NB").val(response.data.Phone_num ?? 0);
        ...
        // ✅ Erreurs selon la longueur
        if (val.length < 18) {
            if (lng === "ar") {
                alert("يجب أن يحتوي الرقم على 18 رقمًا");
            } else {
                alert("Le numéro doit contenir exactement 18 chiffres");
            }
        } else {
            if (lng === "ar") {
                alert("لقد تجاوزت الحد المسموح (18 رقمًا فقط)");
            } else {
                alert("Vous avez dépassé la limite autorisée (18 chiffres seulement)");
            }
        }
                ...
                success: function (response) {
                    ...
                    if (response.status != 302) {
                        ...
                        if (lng == "ar") {
                            ...
                            $("#Prenom_emp").val(
                                response.employe.Prenom_ar_emp
                            );
                        } else {
                            ...
                            if (selectedOption == "RF001") {
                                if (lng == "ar") {
                                    switch (true) {
                                        ...
                                        default:
                                            $("#total_cgj").val(
                                                response.Jour_congé_an + ",أيام"
                                            );
                                            break;
                                    }
                                } else {
                                    $("#total_cgj").val(
                                        response.Jour_congé_an + " ,Jour(s)"
                                    );
                                }
                            }
                            if (selectedOption == "RF002") {
                                $("#total_cgj").val(
                                    response.Jour_congé_mal +
                                        " ,Jour(s) de maladie"
                                );
                            }
                        });
                        if (type != "Maladie") {
                            ...
                            $("#total_cgj").val(
                                response.Jour_congé_mal + ",Jour(s) de maladie"
                            );
                        }
                        if (type != "Maladie") {
                        ...
                        if (
                            response.Jour_congé <= 0 &&
                            document.getElementById("typ_cg").value == "RF001"
                        ) {
                            ...
                            $("#date_rec").val(
                                response.employe.date_recrutement
                            );
                            $("#date_op").val(
                                "01-06-" + currentTime.getFullYear()
                            );
                        } else {
